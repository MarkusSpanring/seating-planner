...
- **Table Details:** Click on a table's center circle to open the **Table Detail Modal**. Here you can:
  - Rename the table.
  - Change seating capacity (seats automatically re-number and shrink down as long as occupancy allows).
  - Enable "Fixiert" lock (turns seats green indicating assignments are final).
  - Delete the table.

### 2. Managing Guests
Guests can be added individually via the **"➕ Gast hinzufügen"** button (only available in Full Table View) or via bulk CSV import. 
- **Guest Properties:** 
  - *Diet / Allergies*: Color-coded for visualization.
  - *Age Category*: Used for stats and pricing breakdowns.
...
To make family assignments easier, you can bind guests together into Family Groups!
- **Group Families Toggle:** When enabled in the Full Table View, linked guests visually clump together into cards.
- **To Create a Family:** In the spreadsheet view, find the "Familie" column and search/select another guest's name. They will be linked into a new family.
- **Bulk Table Assignment:** If grouped guests are all placed successfully, a dropdown appears on their card in the split-view letting you effortlessly reassign the whole family to another table simultaneously. You will get a warning pill if members are awkwardly split across different tables.
